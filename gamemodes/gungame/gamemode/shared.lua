...
function GM:PlayerDeath( victim, inflictor, ply )
    if ply:IsPlayer() and IsValid(ply) and ply ~= victim then
        ...
        if actualItem >= table.Count(weaponies) then
            PrintMessage(HUD_PRINTTALK, ply:Name().." wygrał!")
        else
        	PrintMessage(HUD_PRINTTALK, ply:Name().." - "..actualItem.."/"..table.Count(weaponies))
            ply:SetNWInt("actualItem", actualItem + 1)
            ply:RemoveAllItems()
            local weapon = ply:Give(weaponies[actualItem])
...
function GM:PlayerLoadout( ply )
    local weapon = ply:Give(weaponies[ply:GetNWInt("actualItem", 1)])
    ply:GiveAmmo( 999,  weapon:GetPrimaryAmmoType())
	return true
end
